...
class TrackingNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self._synchronizer = message_filters.ApproximateTimeSynchronizer(
            (image_sub, detections_sub), 10, 0.5)#sub two message nearly simultanous
        self._synchronizer.registerCallback(self.detections_cb)# each time receive 2 two message cal callback
    ...
    def detections_cb(self, img_msg: Image, detections_msg: DetectionArray) -> None:
        ...
        tracked_detections_msg.number = detections_msg.number
        print(f"number: {detections_msg.number}")

        # convert image
        cv_image = self.cv_bridge.imgmsg_to_cv2(img_msg) # transform img data to cv2 type

        # parse detections
        detection_list = []
        detection: Detection # set detection with class Detection
        ...
        if len(detection_list) > 0:
            ...
            if len(tracks) > 0:
                ...
                for t in tracks:
                    ...
                    tracked_box = Boxes(
                        t[:-1], (img_msg.height, img_msg.width)) # t[:-1] take all elements except the last one

                    tracked_detection: Detection = detections_msg.detections[int(
                        t[-1])] # not get the last element to keep purely tracking-related data
                    ...
                    print(f"x: {tracked_detection.bbox.center.position.x }, y: {tracked_detection.bbox.center.position.y} ")
